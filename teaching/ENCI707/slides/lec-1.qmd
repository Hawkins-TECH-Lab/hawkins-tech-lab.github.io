...
- **Reflexivity**: $A ~ A$ - You are indifferent between copies of the same product
- **Transitivity**: $A \succsim B \wedge B \precsim C -> A \succsim C$ - If you prefer A over B and B over C, then you must prefer A over callout
...
- **Utility Function**: Summary of a cosumer's behaviour by means of a mathemnatical function
...
- **Marshallian (Uncompensated) Direct Demand**: Demand for items that **maximizes the utility of consumption** subject to a constrained budget​
- **Hicksian (Compensated) Indirect Demand**: Demand for items that **minimizes expenditure** to attain a certain (fixed) level of consumption utility

## Demand Functions​ {.smaller}
- Function explaining quantity demanded as function of price, income (or total available budget), utility, etc.​
...
- If quantity demanded is expressed as a **function of prices and income** (or total available budget), called **Marshallian** or **direct demand function**
...
$\frac{dx_1(p_1,p_2,I)}{dI} > \frac{q_1}{I}$ -> Superior Good $\eta > 1$
...
- If quantity demanded is expressed as a **function of prices and utility**, called **Hicksian** or **indirect demand function**
- Hicksian demand tells us what consumption bundle achieves a **target level of utility** and **minimizes total expenditure**
  - As it is based on utility, Hicksian demand is **not directly observable**
...
$$\frac{\partial x_1}{\partial p_2} = \underset{\text{Substitution Effect}}{\frac{\partial h_1(p_1,p_2,u)}{\partial p_2}} -  \underset{\text{Income Effect}}{\frac{\partial q_1}{\partial I}x_2(p_1,p_2,I)}$$
...
$$\frac{\partial U(x_1,x_2)}{\partial q_1} = \lambda p_1 \text{ and } \frac{\partial U(x_1,x_2)}{\partial x_2} = \lambda p_2$$
...
## Indirect Utility Function​ {.smaller}
- Indirect utility optimization is the dual of primal direct utility optimization (**expenditure function**)
$$V(p_1,p_2,I) = \min_{p_1,p_2} p_1x_1 + p_2x_2 - \lambda (\bar{U} - U(x_1,x_2))$$
$$\text{s.t. } U(x_1,x_2) \ge \bar{U}$$

- Equivalently, by duality (**indirect utility function**)
...
- Connects **direct utility function** and **indirect utility function**
- Similarly, connects **Marshallian (utility maximization) demand function** and **Hicksian (expenditure minimization) demand function**
...
$$\frac{\partial L}{\partial q_1} = 0 \text{->} \alpha x_1^{\alpha - 1}x_2^{\beta} = \lambda p_1$$
...
- **Elasticity of substitution ($\sigma$)**: Measures the curvateure of the indifference (isoquant) curve
...
$$x_1^*(p_1,p_2,I) = \frac{\alpha I}{p_1 (\alpha + \beta)} \text{(similar result for )} q_2^*(p_1,p_2,I)$$
