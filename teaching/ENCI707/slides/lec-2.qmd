---
title: "Lecture 2 - Aggregate Demand Models"
...
::: incremental
1.  Some econometric tools used in demand modelling:
    - Normal regression
...
- Probabilistic application through simulation to propogate uncertainty
...
- Estimation by maximum likelihood assuming normal cdf
...
- If we assume $\epsilon_i$ follows a normal distribution with **zero mean** and $\sigma^2$ **variance**, then
    ...
    $$Pr(y_i=j|x_i) = \Phi((\tau_j - \beta x_i)/\sigma) - \Phi((\tau_{j-1} - \beta x_i)/\sigma) $$
...
$$Pr(y_i=4|x_i) = 1 - \exp(\tau_3 - \beta x_i)/(1 + \exp(\tau_3 - \beta x_i))$$
...
- Excess zeros arise from **two processes**:
  - **Structural zeros** from a binary model
  - **Sampling zeros** from the ordered model (not truncated)

- Mixture structure:
  $$
  P(Y_i = 0) = \pi_i + (1-\pi_i)F(\tau_0 - \eta_i)
  $$
  $$
  P(Y_i = j>0) = (1-\pi_i)\left[F(\tau_j - \eta_i) - F(\tau_{j-1} - \eta_i)\right]
  $$

- Components:
  - Structural-zero probability: $\pi_i = \Phi(\gamma z_i)$
  - Ordered latent index: $\eta_i = \beta x_i$

- Likelihood:
  $$
  L_i =
  \left[\pi_i + (1-\pi_i)F(\tau_0 - \eta_i)\right]^{I(y_i=0)}
  \left[(1-\pi_i)\left(F(\tau_j - \eta_i) - F(\tau_{j-1} - \eta_i)\right)\right]^{I(y_i=j>0)}
  $$

---

## Ordered Hurdle Regression {style="font-size: 0.6em;"}

- Zeros arise from **one process only** (binary hurdle)
- Positive ordered outcomes arise from a **zero-truncated ordered model**
  $$
  P(Y_i = 0) = 1 - p_i
  $$
  $$
  P(Y_i = j>0) = p_i \cdot 
  \frac{F(\tau_j - \eta_i) - F(\tau_{j-1} - \eta_i)}
       {1 - F(\tau_0 - \eta_i)}
  $$

- Components:
  - Hurdle probability:  $p_i = \Phi(\gamma z_i)$
  - Ordered latent index: $\eta_i = \beta x_i$
...
- Likelihood:
  ...
  \left[p_i \cdot 
  \frac{F(\tau_j - \eta_i) - F(\tau_{j-1} - \eta_i)}
       {1 - F(\tau_0 - \eta_i)}\right]^{I(y_i=j>0)}
  $$
...
- Count of events occurring **randomly** and **uniformly** in time with constant rate:
  $$
...
- Issues:
  - Heterogeneity violates constant-rate assumption
  - Upper limits → over/under dispersion

---

## Poisson Regression with Heterogeneity {.smaller}

- Add gamma heterogeneity:
  $$
  \theta_i^* = \exp(\beta x_i + u_i)
  $$
  $$
  u_i \sim \text{Gamma}(1, 1/\theta)
  $$

- Negative Binomial pmf:
  $$
  P(Y_i = y_i) =
  \frac{\Gamma(\theta + y_i)}{\Gamma(y_i +1)\Gamma(\theta)}
  r_i^{y_i}(1-r_i)^{\theta}
  $$
  $$
  r_i = \frac{\exp(\beta x_i)}{\exp(\beta x_i) + \theta}
  $$
...
- Excess zeros arise from **two processes**:
  - Structural zeros (binary model)
  - Sampling zeros (Poisson/NB)

- Mixture structure:
  $$
  P(Y_i = 0) = \pi_i + (1-\pi_i)f(0)
  $$
  $$
  P(Y_i = j>0) = (1-\pi_i)f(j)
  $$

- Components:
  - Structural-zero probability:$\pi_i = \Phi(\gamma z_i)$
  - Count model: $f(j) = \text{Poisson or NB pmf}$

- Likelihood:
  $$
  L_i =
  \left[\pi_i + (1-\pi_i)f(0)\right]^{I(y_i=0)}
  \left[(1-\pi_i)f(y_i)\right]^{I(y_i>0)}
  $$
...
- In transportation modelling, analysis deals both directly and  indirectly with decision-making of people and firms
...
## Modelling Passenger Travel Demand {.smaller}
- Urban travel demand:
...
- Modelling multiple dimensions of demand: number of trips, time of trips, mode of trips, etc.
    - Urban Transportation Modelling System (UTMS): trip-based model, activity-based models
- Modelling specific dimension of trips: total trips, transit ridership, etc.
...
- Aggregate versus disaggregate modelling:
    ...
    - In most cases aggregate models can **complement** disaggregate modelling approaches by making the trends better understood
...
$$\ln(V_C/V_A) = \beta_{0CA} + \sum_{i=1}^n \beta_{iBA}(x_{iC} - x_{iA}) + \sum_{j=1}^n \gamma_{jC}y_j + \epsilon_{CA}$$
