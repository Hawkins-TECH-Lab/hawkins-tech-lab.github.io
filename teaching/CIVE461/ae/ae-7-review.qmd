...
$$V=\alpha T_a^{-2.0} T_t^{+0.5} C_a^{-0.5} C_t^{+0.6}$$
...
where y household peak period trips. $x_1$ is the number of workers in the household and $z_1$ is a dummy variable that takes the value of 1 if household income is over 70,000 and 0otherwise. The second was cross-classification analysis model based on two income strata (high and low income) and two levels of family income (1 or less and 2 or more workers per households). The estimated trip rates are:
