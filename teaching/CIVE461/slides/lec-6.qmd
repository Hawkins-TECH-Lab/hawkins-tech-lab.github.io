...
## Mode Choice in 4-Step Model Framework
::: columns
::: {.column width="50%"}
```{mermaid}
flowchart TB
  A[Mode Choice] ---->|Trip End Model| B(Trip Distribution)
```
:::
::: {.column width="50%"}
```{mermaid}
flowchart TB
  A[Trip Distribution] ---->|Trip Interchange Model| B(Mode Choice)
```
:::
:::

## Trip-End Models
- Trip end mode split (**choice**) models apply before trip distribution and split *trip ends* estimated in trip generation among ends (**destinations**)
...
- Works better for **short-term** and when there is **no modal competition** & traffic congestion – small urban & rural areas where transit is a *social service*
...
- Trip interchange mode split (**choice**) models apply after trip distribution and split *trip interchanges* estimated in trip distribution
- Since O-D flows are known, **can compute travel times, costs, etc.** for competing modes
- Trip interchange models generally applied in medium to large urban areas where alternative modes are competitive and traffic congestion is a factor
...
- **At a minimum**, requires some representation of competition between auto and transit

## Auto Mode Representation
- Common to distinguish between drivers and passengers:
...
- Alternatively, we can distinguish between drive alone and shared trips
...
- Combined auto-transit modes exist, in which auto is used to access transit system
  ...
  - **Kiss & Ride:** trip-maker is driven as a passenger to a transit station and is dropped off there
...
![](../images/lecture6/decision-structure.png){.absolute bottom="0" right="300" width=75%}
...
COSTm = out-of-pocket travel cost ($) for mode m
IVTTm = in-vehicle travel time (min.) for mode m
OVTTm = out-of-vehicle travel time (min.) for mode m
NVEH = avg. no. of vehicles per household in home zone
TWY = 1 if employment zone is located within the catchment aera of a transitway station outside the CBD
           = 0 otherwise
...
- There are **alternative specific constants** for drive and passenger modes but not transit
...
- Socio-economic variables **do not vary** between alternatives
  ...
  - If included in model as generic variables, would add same utility to all alternatives and have no impact on choice probability
...
$$𝑉𝑇𝑇=𝛽_𝑡𝑖𝑚𝑒/𝛽_𝑐𝑜𝑠𝑡 =−(−0.20)/(−0.57)=$21/ℎ𝑜𝑢𝑟$$

- Similarly, VTT for OVTT = $31.6/hour
- **Question:** How do you think WTP for IVTT compares for auto vs. transit modes?
...
Problem: extremely expensive to survey all individuals and difficult to simulate them
...
Divide workers into “n” NVEH and DLIC categories
...
![](../images/lecture6/util-func.png){.absolute bottom="0" right="300" width=75%}
...
![](../images/lecture6/tot-enum.png){.absolute bottom="0" left="0" width=75%}

![](../images/lecture6/naive-agg.png){.absolute bottom="0" right="0" width=75%}
...
![](../images/lecture6/class-enum.png){.absolute bottom="0" right="0" width=75%}
...
![](../images/lecture6/dest-choice-1.png){.absolute bottom="0" right="0" width=75%}
...
![](../images/lecture6/dest-choice-2.png){.absolute bottom="0" right="0" width=75%}
...
![](../images/lecture6/mode-dest-choice.png){.absolute bottom="0" right="0" width=75%}
