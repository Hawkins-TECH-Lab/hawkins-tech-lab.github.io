...
1. Set $k = 1$. Choose a node as the **initial node**. Label this node [-,0] where the minus sign indicates there is **no predecessor node** & the zero indicates there is **zero cost** associated with reaching this node. Set M equal to {n1} where n1 is equal to the node number for the initial node.
...
## Shortest Path - Dijkstra Algorithm 2 {.smaller}
4. Find an unlabeled node $n_k$ such that $C(nk) = min {C(n_x) + min[(n_x, n_y)]}$ for $n_x \in M$ & $n_y \in M’ = C(n_1) + c(n_i, n_k)$ for some $n_i \in M$, $n_k \in M’$. Assign the label $[n_i, C(n_k)]$ & add $n_k$ to $M$. 
    - The equation above searchers over the currently labeled nodes – all nodes connected to initial node with links to unlabeled nodes. Seeks new node to connect to initial node (i.e., add to labeled set), such that path to new node has **lowest travel cost** from initial node.
...
- **Discrete route choice models** (e.g., logit) exist
  ...
  - Need to take care in defining choice et of feasible routes
...
- **III:** **Highly congested situation**, where small increase in volume causes larger increase in time.
  - Penalty for wrong information would incur high delay. Assuming that users are fairly aware of situation, deterministic assignment would be more appropriate.
...
![](../images/lecture7/aon_1.png){.absolute top="275" right="0" width=40%}
![](../images/lecture7/aon_1.png){.absolute bottom="0" right="100" width=40%}

## All-Or-Nothing Assignment: Example
Sum all assigned trips to get complete assignment

![](../images/lecture7/aon_4.png){fig-align="center"}
